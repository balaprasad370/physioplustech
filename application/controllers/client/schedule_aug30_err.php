...
class Schedule extends CI_Controller {
	...
	public function cancel_appointments() {
		...
		if($result != ''){
			...
			// $message_doctor = 'Dear '. ucfirst($first_name).','.ucfirst($clinic_name). '.Your Appointment time was rescheduled.';
					 $smsUrl = 'https://www.smsgateway.center/SMSApi/rest/send?userId=klakshman&password=Smsgateway@2018&senderId=FISIYO&sendMethod=simpleMsg&msgType=TEXT&mobile='.$response['mobile'].'&duplicateCheck=true&format=json&msg='.urlencode($str);
					 $sms['doctor'] = 'Done';
					$sms_count+=1;
