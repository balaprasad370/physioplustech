...
class Patient extends CI_Controller {
	...
	public function view() { 
		...
		$data['treatmentdys'] = $this->patient_model->viewTreatmentsDyas($patient_id);
	...
	public function file_upload_old(){
	...
	public function file_upload(){
		...
		{
			$photo = $patient_id.'.png';
			$output_file = 'uploads/inves/'.$patient_id.date('YmdHis').'.png';
			$base64_string = $this->input->post('img_data');
	...
	public function photo_upload(){
		$patient_id = $this->uri->segment(4);
		$result =array();
		//print_r($this->input->post('img_data'));exit;
		if (!$this->input->post('img_data'))
		{
			$result['status'] ='fail';
			$result['msg'] ='File Upload Error';
		...
		{
			$photo = $patient_id.'.png';
			$output_file = 'uploads/patient/'.$patient_id.'.png';
			$base64_string = $this->input->post('img_data');
			$ifp = fopen( $output_file, 'wb' ); 
			$data = explode( ',', $base64_string );
			fwrite( $ifp, base64_decode( $data[ 1 ] ) );
	...
	public function photo_upload_old(){
		$patient_id = $this->uri->segment(4);
		$result =array();
   ...
   public function add_exercise_protocol() {
	...
	public function consent_delete(){
		...
		public function invoice_new_days()
	{
		$data['page_name'] = 'patient';
		$this->load->helper(array('form'));
		//$segment_array = $this->uri->segment_array();
		...
		if($query = $this->invoice_model->groupInvoice_days($treatment,$patient_id,$group_id))
		{
