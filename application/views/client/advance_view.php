...
<head>
    ...
    <meta http-equiv="Content-Type" content="text/html; charset=utf-8"/>
    <title> Advance List - Physio Plus Tech</title>
                        ...
                        <div class="row" ng-controller="customersCrtl">
                            <div class="col-md-12">
                                <div class="main-card mb-3 card">
                                    <div class="card-header"> Advance List
                                   </div>
								    <div class="col-md-12" >
										...
										<tr><td colspan="9"> <input style="width:100%;" type="text" ng-model="search" ng-change="filter()" placeholder="Search All Fields" class="form-control" /> </td>
											...
											<tr ng-if="data != 0" ng-show="filteredItems > 0" ng-repeat="data in filtered = (list | filter:search | orderBy : predicate :reverse) | startFrom:(currentPage-1)*entryLimit | limitTo:entryLimit" id="data.bill_id">
												...
												<td class="text-center">{{data.advance_amount}}</td>
												 ...
												 <button class="mb-2 mr-2 btn-icon btn-icon-only btn-shadow btn-dashed btn btn-outline-danger delete" data-toggle="tooltip" data-placement="top" title="Delete" ng-click="delete(data.advance_id)"><i class="fa fa-trash-alt"> </i></button>
												</td>
												</tr>
...
app.controller('customersCrtl', function ($scope, $http, $timeout) {
    $http.get('<?php echo base_url().'client/bill/getadvanses'; ?>').success(function(data){
        $scope.list = data;
	...
	$scope.delete = function($advance_id) {
		var utl= '<?php echo base_url().'client/advance/delete' ?>';
		swal({
          title: "Are you sure?",
          text: "You will be able to delete this Advance Record!",
          type: "warning",
			...
			$.ajax({
				...
				success:function(data, textStatus, jqXHR,form) 
				{
					if(data.status == 'success') {
					  swal("Deleted!", "Your Advance Records has been deleted!", "success");
					}
					setTimeout(function(){ 
