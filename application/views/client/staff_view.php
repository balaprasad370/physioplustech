													...
													<div class="">
														...
														<tr><td colspan="9"> <input style="width:100%;" type="text" ng-model="search" ng-change="filter()" placeholder="Search All Fields" class="form-control" /> </td>
															...
															<tr ng-if="data != 0" ng-show="filteredItems > 0" ng-repeat="data in filtered = (list | filter:search | orderBy : predicate :reverse) | startFrom:(currentPage-1)*entryLimit | limitTo:entryLimit" id="data.bill_id">
																...
																<br>{{data.first_name}}
																<br>{{data.designation_name}}
																</td>
															
																<td class="text-center">{{data.mobile_no}}
																	<br><span style="white-space: normal; max-width: 120px; overflow: hidden; text-overflow: ellipsis; display: inline-block;">{{data.email}}</span>
																</td>
												...
												<div class="col-md-12">
													<div class="main-card mb-3">
														...
														<div class="table-responsive">
															...
															<?php if($this->session->userdata('staff_id') == false){ ?><th class="text-center">Action</th> <?php } ?>
															  </tr>
															</thead>
															<tbody>
															<tr><td colspan="9"> <input style="width:100%;" type="text" ng-model="search" ng-change="filter1()" placeholder="Search All Fields" class="form-control" /> </td>
																...
																<tr ng-if="data != 0" ng-show="filteredItems > 0" ng-repeat="data in filtered = (list | filter:search | orderBy : predicate :reverse) | startFrom:(currentPage-1)*entryLimit | limitTo:entryLimit" >
																	<td class="text-center">{{data.first_name}}

																		<br class="text-center text-muted"> 
																		<a href="<?php echo base_url().'client/staff/edit/'; ?>{{data.staff_id}}"><div class="badge badge-pill badge-info">{{data.staff_code}}</div></a>
																		<br>{{data.created_date.split(' ')[0]}}
																	</td>
																
														
																	<td class="text-center">{{data.mobile_no}}
                                                                     <br> <span style="white-space: normal; max-width: 120px; overflow: hidden; text-overflow: ellipsis; display: inline-block;">{{data.username}}</span>
                                                                     <br> <a ng-if="data.status == 0" ><button ng-click="active(data.user_id,data.first_name)" type="submit" class="mb-2 mr-2 btn-pill btn btn-alternate status">Inactive</button></a>
															...
															<?php if($this->session->userdata('staff_id') == false){ ?>			<td class="text-center">
