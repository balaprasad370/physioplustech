    ...
    outline: 0;
            ...
            <div class="app-main__outer">
                <div class="app-main__inner">
					<?php if($this->uri->segment(5) != false && $this->uri->segment(5) != 'page' && $this->uri->segment(5) != 'date' && $this->uri->segment(5) != 'treatments') {?>
                                       ...
                                       <div class="row">
											...
											<div class="col-md-6">
                                                 ...
                                                 <div class="position-relative form-group"><label for="case_notes" class="">Staff Name *</label>
													 ...
													 <?php  
															...
															if($sid != false && $sid != 'page' && $sid != 'date' && $this->uri->segment(5) != 'treatments') { 
											...
											<div class="row staff_id2">
												<div class="col-md-6">
											     ...
											     <input type="hidden" class="form-control" id="treatment_idss" name="treatment_idss">
...
$(document).ready(function() {
	...
	$('.bill').hide();
		...
		var treatment_id = '<?php echo $this->uri->segment(6); ?>';
		if(val != 'page' && val != 'date'){
			...
			var sn1 = sn.replace("/", "-");
			var provD = p_id + '/' + sn1+'/'+treatment_id; 
			...
			$.ajax({
					...
					success: function(data) {
						...
						alert('error');
								...
								$('#treat').val(opt[2]);
								$('#staff_id').val(opt[4]);
								$('#treatment_idss').val(treatment_id);
								$('#staf').val(opt[5]);
								if(opt[0] == '1'){
									$('.bill').hide();
								} else {
					...
					success: function(data) {
						...
						alert('error');
							...
							//	$('#staff_ids').parsley().destroy();
								...
								//$('#staff_ids').parsley('destroy').refresh();
								//$('#staff_ids').removeClass('parsley-error');
								/* $('#staff_ids').parsley('addConstraint', {
									required: false 
									}); */
								...
								$('.staff_id2').hide();
								$('.staff_idss').show();
								//$("#staff_ids").parsley('required', 'true');
