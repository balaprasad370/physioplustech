...
<head>
	...
	<meta property="og:description" content="Please Share your valuable feedback" />
...
<body>
	...
	<div class="app-container app-theme-white body-tabs-shadow fixed-header fixed-sidebar">
																									...
																									<div class="add_new_patient">
																										<div class="" style="text-align: left !important;">
                                                                                                         ...
                                                                                                         <?php 	$today=date('Y-m-d');
																												 $this->db->select('*')->from('tbl_appointments')->where('client_id', $this->session->userdata('client_id'));
													$this->db->where('appointment_from >=', $today);
													$this->db->where('status =', 0);		 
 															$query = $this->db->get();
															$num=$query->num_rows();
															//echo "status : ".$query->row()->status;
															if($num>0)
															{
	   ...
	   <?php //foreach ($session as $row) {?>
															 	   ...
															 	   $patient_id = $rowd->patient_id;
																  	$appointment_id=$rowd->appointment_id;
																   $staff_id = $rowd->staff_id;
																   $start=$rowd->start;
																   $end=$rowd->end;
																   $visit=$rowd->visit;
																    $this->db->select('first_name,last_name')->from('tbl_patient_info')->where('patient_id', $patient_id);
															...
															$query = $this->db->get();
																	...
																	$last_name = $query->row()->last_name;																						
																													...
																													?>
																														<div> <?php echo $first_name.' '.$last_name; ?>  </div><?php //} ?>
																													</div>  
																													<div class="col-6 p-1 arrow">
																													<a href="<?=base_url()?>application\views\client\view_patient.php?id=<?php echo $appointment_id;?>" class="fa fa-angle-left" aria-hidden="true"></a>
																													</div>
																												</div>
																												<div class="row">
																													<div class="col-5 p-1">
									...
									<?php  $this->db->select('*')->from('tbl_staff_info');
										$this->db->where('staff_id',$staff_id);
																													...
																													<div class="col-4 p-1">
																														<small>Time</small>
															<div><?php 
															echo date('H:i',strtotime($start)).' - ' .date('H:i',strtotime($end));
															...
															?></div>
																													...
																													<div class="col-3 p-1" style="text-align: right !important;">
																														...
																														<div class="badge badge-pill badge-danger"></div><?php if($visit != '1'){ ?>
												...
												<?php if($row['visit'] != '1'){ 
													if($row['item_id']!=0){
														?>
														<a class="mb-2 mr-2 btn-icon btn-icon-only btn-shadow btn-dashed btn btn-outline-success daily_reg" data-toggle="tooltip" data-placement="top" title="Change Visit Status" href="<?php echo '#'.$row['appointment_id'].'#'.$row['pname'].' '.$row['pl_name']; ?>"><i class="fa fa-check"></i></a>
														<input type="hidden" value="<?=$row['google_review']?>" id="google_review" name="google_review">
												  <input type="hidden" value="<?=$this->session->userdata('review')?>" id="g_link" name="g_link">
																 
												  <div  id="feedbackModal"  class="modal hide feedbackModal" data-backdrop="static" data-keyboard="false" style="margin-top:100px;style:none">
													<div class="modal-dialog modal-md" >
														<div class="modal-content">
															...
															<div class="modal-body" style="padding: 2.3rem;">
																...
																<input type="hidden" value="<?=$row['mobile_no']?>" id="p_mobile" name="p_mobile">
																<input type="hidden" value="<?=$row['pname']?>" id="p_name" name="p_name">
																<input type="hidden" value="<?=$row['patient_id']?>" id="patient_id" name="patient_id">
																 <input type="hidden" value="<?=$row['staff_id']?>" id="staff_ids" name="staff_ids">
															...
															<div class="modal-footer">
																<a type="button" class="btn btn-primary sendFeedBackLink" id="sendFeedBackLink" >Send Link</a>
...
<script type="text/javascript" src="<?php echo base_url() ?>assets/js/jquery-ui-1.11.1.js"></script>
	...
	<script type="text/javascript" src="<?php echo base_url() ?>assets/js/jquery-ui-slider-pips.js"></script>
...
<script>
	...
	var p_mobile = row.find("#p_mobile").val();
	var p_name = row.find("#p_name").val();
	var p_id = row.find("#patient_id").val();
	var d_id = (row.find("#staff_ids").val() !="" && row.find("#staff_ids").val()>0) ? row.find("#staff_ids").val() : $("#staff_id").val();
        ...
        function(){
		    ...
		    $.ajax({
				...
				{ 
						...
						{
						  swal("Success!", name + " status has been changed!", "success",function() {
							//alert("test");
							});
						
						if(row.find("#google_review").val()==0)
						{
							if(row.find("#g_link").val()!='')
							{
							var g_link = row.find("#g_link").val();	
							swal({
									  //title: "Are you sure?",
									...
									function(inputValue){
										...
										var msg1='https://web.whatsapp.com/send?phone=91'+p_mobile+'&text=Hi '+p_name+','+'%0a'+'Hope you had a good Physiotherapy session at '+clinic+'%0a'+'%0a'+'We wish you a speedy recovery!'+'%0a'+'%0a'+'If you are happy with our service, Kindly click below to leave a review.'+'%0a'+g_link+'%0a'+'%0a'+'Thank you'+smiley;
							...
							window.location.href="<?php echo base_url()?>client/dashboard/home";
						}
						
					}/* else{ */		 
						row.find("#feedbackModal").modal({backdrop: 'static',keyboard: false}); 
					/* } */
				 }	
				},
	...
	var feedbacktype = row.find("input:radio[name='feedbacktype']:checked").val();
	var p_mobile = row.find("#p_mobile").val();
	var p_name = row.find("#p_name").val();
	var p_id = row.find("#patient_id").val();
	var d_id = (row.find("#staff_ids").val() !="" && row.find("#staff_ids").val()>0) ? row.find("#staff_ids").val() : $("#staff_id").val();
