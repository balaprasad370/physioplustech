																			...
																			<?php foreach($result as $row){ 
																				...
																				<tr>
																					<td class="text-center">
																						...
																						<br>
																						<?php echo date('d-m-Y',strtotime($row['ex_date'])); ?>
																						<br>
																						<span style="white-space: normal; max-width: 120px; overflow: hidden; text-overflow: ellipsis; display: inline-block;"><?php echo $row['email'] ?></span>
																					</td> 

																					<td class="text-center text-muted"> 
																						<div style="white-space: normal; max-width: 120px; overflow: hidden; text-overflow: ellipsis; display: inline-block;" class="badge badge-pill badge-info"><?php echo $row['chartname']; ?></
																						<div>
																						<br>
																						<?php echo $row['pay'] ?>
																					</td>  
																					...
																					<td class="text-center">
																						<a href="<?php echo 'https://web.whatsapp.com/send?phone=91'.$mobile.'&text=Dear '.$row['patient_name'].', This message is a reminder about the exercise plan prescribed by '.$this->session->userdata('clinic_name').' to you. Kindly adhere to the plan for better results. Kindly click the link below to access the exercise chart. We wish you good health and wellness from the team of '.$this->session->userdata('clinic_name').' '.base_url().'pages/default_pdf_verification/'.$this->session->userdata('client_id').'/'.$row['patient_id'].'/public-'.$verifycode; ?> " target="_blank" >
																							<button class="mb-2 mr-2 btn-icon btn-icon-only btn-shadow btn-dashed btn btn-outline-success delete" data-toggle="tooltip" data-placement="top" title="Whatsapp" ng-click="send(data.start,data.mobile_no,data.staff_name)">
																								<img src="<?php echo base_url().'img/whatsapp Logo.png' ?>" style="width:23px; height:23px;" > </img>
																							</button>
																						</a>
																					</td>
																				</tr>
