								...
								<h5 class="card-title"> Reschedule 
                                    ...
                                    <form  action="<?php echo base_url().'client/schedule/update_appointment' ?>" method="post" parsley-validate role="form">
                                       <?php if($sch_slot != false){ foreach($sch_slot as $sch){ ?>
											...
											<?php } else {?>
											<input type="hidden" name="date" class="chosen-select chosen-transparent form-control" id="date" value="" /> 
											<input type="hidden" name="date1" class="chosen-select chosen-transparent form-control" id="date1" value="" />
											<?php } ?>	
                                            ...
                                            <div class="col-md-6">
                                                <div class="position-relative form-group"><label for="case_notes" class="">Patient Name</label>
											     <input type="text" name="first_name" readonly value="<?php echo $row['title'] ?>"   class="form-control" placeholder="Enter First Name" parsley-trigger="change" parsley-required="true"  id="first_name">
												</div>
											</div>
											<div class="col-md-6">
												...
												<input type="hidden" value="<?php echo date('d/m/Y',strtotime($row['appointment_from'])); ?>" id="appointment_from_old" name="appointment_from_old">
											</div>
											...
											<div class="col-md-6">
                                                 <div class="position-relative form-group"><label for="case_notes" class="">Start Time</label>
													 ...
													 <input type="hidden" value="<?php echo date('h:i a',strtotime($row['start'])); ?>" id="appointment_time_old" name="appointment_time_old">
												 </div>
											</div>
											<div class="col-md-6">
											...
											<div class="clearfix">
												...
												<a href="<?=base_url()?>client/schedule/appointment" class="mb-2 mr-2 btn btn-pill btn-danger">Cancel</a>
												</center>
                                            </div>
	 ...
	 $('form').on('submit', function (event) {
		    ...
		    //window.alert(clinic_map);
		     var map=clinic_map.replaceAll("+","%2B");
				var appointment_time_old = $("#appointment_time_old").val();
				var appointment_from_old = $("#appointment_from_old").val();
			  ...
			  $.ajax({
						...
						success:function(data, textStatus, jqXHR,form) 
						{
								...
								function(inputValue){
									...
									//window.location.href = "'<?php echo base_url()?>'client/schedule/appointment";
									window.location.href = "<?php echo base_url()?>client/schedule/appointment";
										...
										//var msg='Dear '+data.patientname+','+'%0a'+ 'Your appointment with '+data.staffname+' on '+appointment_from_old+' '+appointment_time_old+'  has been rescheduled to '+data.date+' '+data.time+' successfully';

										}
										else{
											//var msg='Dear '+data.patientname+','+'%0a'+ 'Your appointment with '+data.staffname+' on '+data.date+' '+data.time+'  has been rescheduled successfully .'+'%0a'+'Visit us:'+'%0a'+map ;
										//	var msg='Dear '+data.patientname+','+'%0a'+ 'Your appointment with '+data.staffname+' on '+appointment_from_old+' '+appointment_time_old+'  has been rescheduled to '+data.date+' '+data.time+' successfully .'+'%0a'+'Visit us:'+'%0a'+map ;
										}
