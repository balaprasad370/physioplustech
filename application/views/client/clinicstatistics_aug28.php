...
<body>
	...
	<div class="app-container app-theme-white body-tabs-shadow fixed-header fixed-sidebar">
	   ...
	   <?php foreach ($session as $row) {?>
																		...
																		$CK =& get_instance();
																		$CK->load->model(array('registration_model'));
																		$patients_id = $CK->registration_model->get_summary($row['patient_id']);
																											/*		 $this->db->select('patient_id')->from('tbl_appointments')->where('client_id', $this->session->userdata('client_id'));
															 
															$query = $this->db->get();
															
															
																   $patient_id = $query->row()->patient_id;	*/
																    $this->db->select('first_name,last_name')->from('tbl_patient_info')->where('patient_id', $patients_id);
															...
															$query = $this->db->get();
																	...
																	$last_name = $query->row()->last_name;																						
																													...
																													?>
																														<div> <?php echo $first_name.' '.$last_name; ?>  <?php echo $patient_id;?></div><?php } ?>
																													</div>  
