            ...
            <div class="app-main__outer">
                <div class="app-main__inner">
                    ...
                    <div class="tab-content">
                            ...
                            <div class="main-card mb-3 card">
                                <div class="card-body"><h5 class="card-title"> New Invoice
                                    ...
                                    <form class="" action="<?php echo base_url().'client/invoice/add_invoice1' ?>" method="post"  role="form" parsley-validate id="signupForm">
										...
										<div role="group" class="mb-2 btn-group-sm btn-group btn-group-toggle" data-toggle="buttons">
											  ...
											  <div class="col-md-6">
                                                 <div class="position-relative form-group"><label for="mobile_no">Mobile No</label> 
												  <input name="mobile_no" id="mobile_no" type="text" class="form-control" placeholder="Enter Mobile Number" maxlength="8">
												</div>
										...
										<div class="form-row">
                                            <div class="col-md-6">
                                                <div class="position-relative form-group"><label for="vendor" class="">Tax & Discount</label>
												<select  name="perc"  style="width:96%;" class="chosen-select chosen-transparent form-control" placeholder="please Select Item " id="perc">
                                        ...
                                        <table class="mb-0 table" id="InvoiceTable">
											...
											?>
											  ...
											  <td><input type="text"  class="form-control tax" value="0" name="tax[]" ></td>
											  <td><input type="text"  class="form-control discount" value="0" name="discount[]" ></td>
									...
									<td class="width70" style="line-height: 28px"><strong>$ <font id="SubTotal">0.00</font></strong></td>
									</tr>
									</tbody>
									</table>
									</div>
                        ...
                        <td class="width70" style="line-height: 28px"><input type="text"  class="form-control" id="card_no" name="card_no" readonly  autocomplete="off"></td>
                        </tr>
						<tr>
                        <td class="width30" style="text-align:right; vertical-align: middle">Name of the card</td>
                        <td class="width70" style="line-height: 28px"><input type="text"  class="form-control" id="card_name" name="card_name" readonly  autocomplete="off"></td>
                        </tr>
                        </tbody>
                        </table>
						</div>
...
$(document).on('click', 'a.DeleteRow', function(e){
	...
	$(document).on('change', '[name="item_id[]"]', function(){
			...
			if(val[1] === 'undefined' || val[1] == undefined || val[1] == void 0 ){
				...
				parent.find('[name="tax[]"]').val(0).trigger('keyup');
			   	parent.find('[name="discount[]"]').val(0).trigger('keyup');
			}
		});
    ...
    $(document).on('keyup', '[name="item_quantity[]"]', function(){
			...
			ta = parseFloat( $thisRow.find('[name="tax[]"]').val() );
			disc = parseFloat( $thisRow.find('[name="discount[]"]').val() );
			...
			if(val[1] === 'undefined' || val[1] == undefined || val[1] == void 0 ){
				$thisRow.find('[name="item_amount[]"]').val( (values).toFixed(2) ).trigger('subTotalCalc');
			} 
			else {
				 if(val[0] == 'PR'){
					if(val[2] > quantity){
						...
						$thisRow.find('[name="item_amount[]"]').val( (values2).toFixed(2) ).trigger('subTotalCalc');
					}
				} else { 
					...
					if(parseInt(order) == parseInt(quantity) || parseInt(order) < parseInt(quantity) ){
					   ...
					   $thisRow.find('[name="item_amount[]"]').val(values1).trigger('subTotalCalc');
					 }
					else {
						$thisRow.find('[name="item_amount[]"]').val( (values).toFixed(2) ).trigger('subTotalCalc');
					}
	...
	$(document).on('change', '[name="discount[]"]', function(){
	     var
			$this = $(this),
			$thisRow = $this.parents('tr'),
			disc = ($this.val() == '') ? 0 : parseInt( $this.val() ),
			price = parseFloat( $thisRow.find('[name="item_price[]"]').val() );
	...
	$(document).on('change', '[name="tax[]"]', function(){
	   var
			$this = $(this),
			$thisRow = $this.parents('tr'),
			ta = ($this.val() == '') ? 0 : parseInt( $this.val() ),
			price = parseFloat( $thisRow.find('[name="item_price[]"]').val() );
			quantity = parseFloat( $thisRow.find('[name="item_quantity[]"]').val() );
			disc = parseFloat( $thisRow.find('[name="discount[]"]').val() );
			var id = $thisRow.find('[name="item_id[]"]').val();
