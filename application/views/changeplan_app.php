...
<form class="checkout" action="<?php echo base_url().'razorpay/data1.php' ?>" method="POST">
	...
	<tr>
    	<td height="46"><label>Online Charges</label></td>
		<td>:</td>
	...
	$("#checkrate").click(function(){
		...
		if(plan == 1){
			if(user >1 || location >1){
					...
					$("#price").html('');
					$("#price").html('Rs:'+Math.round(total));
			...
			if(user >1 || location >1){
					...
					sms_price = sms_count * 0.50;
					if(duration == 12){
						price =  ((8000 + parseInt(location_price)) + parseInt(sms_price));
						total = price + (price * 0.145);
					} else if(duration == 3){
						price =  ((2700 + parseInt(location_price)) + parseInt(sms_price));
						total = price + (price * 0.145);
					} 
					else if(duration == 6){
						price =  ((5000 + parseInt(location_price)) + parseInt(sms_price));
						total = price + (price * 0.145);
					}
					else {
					...
					sms_price = sms_count * 0.50;
					if(duration == 12){
						price = (8000 + parseInt(sms_price));
					    total = price + (price * 0.145);
					} else if(duration == 6){
						price = (5000 + parseInt(sms_price));
					    total = price + (price * 0.145);
					} else if(duration == 3){
