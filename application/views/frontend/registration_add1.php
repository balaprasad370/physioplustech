...
<!--<?php include_once("analyticstracking.php")?>-->
    <div class="container">
      <form class="form-signin" action="<?php echo base_url().'sign_up' ?>" method="post" role="form" parsley-validate>
	  <?php
    ...
    <div class="alert alert-danger"> <h5><b><?= $this->session->flashdata('error_msg') ?></b><h5> </div>
<?php } ?>	
	  <h5>if you already have a account click here to</span> &nbsp;|&nbsp; <a href="<?php echo base_url(); ?>client/dashboard/home">Sign In?</a></h5>
		...
		<p>( To Strengthen the security, we have added Mobile and Email Verification.)</p>
        ...
        <div class="login-wrap">
           ...
           <input type="text" name="clinic_name" STYLE="color: #000000; font-family: Verdana; font-weight: bold; font-size: 12px; background-color: #FFFFFF;"class="form-control" placeholder="Clinic name *" parsley-required="true">
			...
			<?php } else { ?>
			<input type="text" name="coupon_code" STYLE="color: #000000; font-family: Verdana; font-weight: bold; font-size: 12px; background-color: #FFFFFF;" class="form-control" disabled value="<?php echo $code; ?>" placeholder="Coupon code" >
			<?php } ?>
...
<?php //echo '<img src="' . $_SESSION['captcha']['image_src'] . '" alt="CAPTCHA code" id="image" style="padding-top:10px">';?>
										<?php
...
<!--<div class="g-recaptcha" data-sitekey="6LdJzJkaAAAAAFFuiZm4dvq_5NcI5nA2-S7NsKp8"></div>-->
      <div id="html_element"></div>
<!--<input type="text" name="txt2" STYLE="color: #000000; font-family: Verdana; font-weight: bold; font-size: 12px; background-color: #FFFFFF;" class="form-control" placeholder="Enter above Captcha " id="txt2" parsley-required="true">
     ...
     <script type="text/javascript">
      var onloadCallback = function() {
        grecaptcha.render('html_element', {
          'sitekey' : '6LdJzJkaAAAAAFFuiZm4dvq_5NcI5nA2-S7NsKp8'
        });
      };
    </script>
  ...
  /* $('form').on('submit', function (event) {	
		 ...
		 var $form = $(this);
		if ( $(this).parsley().isValid() && $("#honeypot").val()=="") {
		...
		$.ajax({
            ...
            success:function(data, textStatus, jqXHR,form) 
			{
				...
				window.location.href="<?php echo base_url().'registration/registration_verify/'?>"
				}, 1000);
			},
			error: function(jqXHR, textStatus, errorThrown) 
			{
			...
			$.gritter.add({title:	'Success',text:	'spam Entry not allow'});
				setTimeout(function(){ 
				$('.md-close btn btn-default').click();
				//window.location.reload();
				}, 1000);
