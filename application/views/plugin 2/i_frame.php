  ...
  <form id="contact" action="<?php echo base_url().'plugin/dashboard/add_rand' ?>" method="post">
	...
	<?php $this->db->select("start")->from('tbl_appointments');
						...
						<?php if($this->session->flashdata('warning')) { ?>
										...
										<?php } ?>
    <h3 text align="center"><strong><?php echo $this->session->userdata('clinic_name'); ?></strong></h3><center><span>Online Appointment Booking Portal</span></center>
    <h5 text align="left"><img src="<?php echo base_url().'uploads/logo/'.$this->session->userdata('logo'); ?>" align="left" height="80px;" width="80px;"></img src></h5>
	<h5 text align="right">Powered by<img src="<?php echo base_url()?>img/NEW LOGO1.png" align="right" height="80px;" width="80px;"></img src></h5>
    <input type="hidden" class="span11" name="status" id="status" value="1"/>
    ...
    <fieldset>
	 ...
	 <fieldset>
      <input type="text" name="amount" placeholder="Amount" id="amount" disabled>
    </fieldset>
	<fieldset>
...
$(document).ready(function() {
	...
	var p_from=$('#peak_from').val();
	var p_to=$('#peak_to').val();
	var p_amt=$('#peak_amt').val();
	var ha_amt=$('#happy_amt').val();
	var p_from1 =p_from.split(",");
	var p_to1 =p_to.split(",");
	var time_ra=$('#time').val();
	var t_from =time_ra.split(" ");
	var t_from1 =t_from[0].split(":");
	if(t_from[1] == 'AM'){
	  var range = t_from1[0];
	} else {
	   h_to1 = parseInt(t_from1[0]) + 12;
	   var range = h_to1;
	}
	for(i=0;i<p_from1.length;i++){
	var p_f =p_from1[i].split(" ");
	var p_t =p_to1[i].split(" ");
	var p_amt1 =p_amt.split(",");
	if(p_f[1] == 'am'){
		var peak_from = p_f[0];
	} else {
		h_to1 = parseInt(p_f[0]) + 12;
		var peak_from = h_to1;
	}
	if(p_t[1] == 'am'){
		var peak_to = p_t[0];
	} else {
		h_to1 = parseInt(p_t[0]) + 12;
		var peak_to = h_to1;
	}
	if(parseInt(range) > parseInt(peak_from) && parseInt(peak_to) > parseInt(range)) {
		$('#amount').val(p_amt);
		break;
	}
	else if(parseInt(range) == parseInt(peak_from)){
		$('#amount').val(p_amt);
		break;
	}
	else if(parseInt(range) == parseInt(peak_to)){
		$('#amount').val(p_amt);
		break;
	}
	else {
