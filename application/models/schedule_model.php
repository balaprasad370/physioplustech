...
class Schedule_model extends CI_Model {
		    ...
		    $query = $this->db->get();
				if($query->num_rows() > 0){
				   ...
				   if($this->session->userdata('client_id') == '1948'){
				    ...
				    $message_patient = 'Dear '.ucfirst($data->PatientName).', Your appointment request at '.ucfirst($data->clinic_name).' is Cancelled to '.$time.' on '.$time1.', if you want to change it, kindly call '.$mobile.'%0aThank You%0aPhysio Plus Tech';
				    $patientSmsURL = 'https://www.smsgateway.center/SMSApi/rest/send?userId=klakshman&password=Smsgateway@2018&senderId=FISIYO&sendMethod=simpleMsg&msgType=TEXT&mobile='.$data->PatientMobile.'&duplicateCheck=true&format=json&msg='.urlencode($message_patient);
					  ...
					  // $clinic_name1 = 'SUNSHINE SUPER SPECIALITY PHYSIOTHERAPY CLINIC';
				       $message_patient = 'Dear '.ucfirst($data->PatientName).', Your appointment at '.ucfirst($data->clinic_name).'  has been cancelled. For any queries kindly call '.$mobile.'%0aThank You%0aPhysio Plus Tech';
				        $patientSmsURL = 'https://www.smsgateway.center/SMSApi/rest/send?userId=klakshman&password=Smsgateway@2018&senderId=FISIYO&sendMethod=simpleMsg&msgType=TEXT&mobile='.$data->PatientMobile.'&duplicateCheck=true&format=json&msg='.urlencode($message_patient);
	...
	public function getappointment(){
		...
		$this->db->select('pi.mobile_no,ai.encounter_type,ai.chat_room,ai.order_id,ai.bill_status,ai.appointment_from,ai.appointment_id,ai.patient_id,ai.visit,ai.bill_id,ai.title,ai.start, ai.end,ai.googleMeet_link as link,si.first_name as staff_name,si.last_name as staff_lname,si.staff_id,ai.item_id')->from('tbl_appointments ai');
	...
	public function add() {
		...
		if($this->session->userdata('googleCalendar')==1)
			...
			$this->calendarNotification($appointment_id);
                }
		...
		if($query->num_rows() > 0){
			$data = $query->row();
               ...
               // $calDesc='Physiotheraphy Appointment of '.$pat_name.' with '.$doc_name;
                $calDesc='Appointment of '.$pat_name.' with '.$doc_name;
  ...
  'conferenceData' => [
	...
	public function smsNotification($appointment_id, $notify_to) {
		...
		if($query->num_rows() > 0){
			...
			if($data->PatientMobile != '' && $notify_to == 'patient') { 
				if($this->session->userdata('client_id') == '1948'){
				 ...
				 // $message_patient = 'Dear '.ucfirst($data->PatientName).', Your appointment with '.ucfirst($data->DoctorName).' at '.ucfirst($clinic_name1).' is confirmed at '.$time.' and '.$time1.', Any change/cancellation kindly inform us at least 12 hours in advance to avoid CHARGES('.$mobile.').';
				   ...
				   $message_patient = 'Dear '.ucfirst($data->PatientName).', Your appointment with '.ucfirst($data->DoctorName).' at '.ucfirst($clinic_name1).' is confirmed at '.$time.' and '.$time1.',For Any change/cancellation kindly inform us at least 12 hours in advance to avoid EXTRA CHARGES('.$mobile.')%0aThank You%0aPhysio Plus Tech';
				  ...
				  $patientSmsURL = 'https://www.smsgateway.center/SMSApi/rest/send?userId=klakshman&password=Smsgateway@2018&senderId=FISIYO&sendMethod=simpleMsg&msgType=TEXT&mobile='.$data->PatientMobile.'&duplicateCheck=true&format=json&msg='.urlencode($message_patient);
					...
					$message_patient = 'Dear '.ucfirst($data->PatientName).', Your appointment with '.ucfirst($data->DoctorName).' at '.ucfirst($data->clinic_name).' is confirmed at '.$time.' and '.$time1.',For Any change/cancellation kindly inform us at least 12 hours in advance to avoid EXTRA CHARGES('.$mobile.') %0aThank You%0aPhysio Plus Tech.';
			...
			if($data->StaffMobile != '' && $notify_to == 'doctor') {
				if($this->session->userdata('client_id') == '1948'){
					 $clinic_name1 = 'SUNSHINE SUPER SPECIALITY PHYSIOTHERAPY CLINIC';	
					$message_doctor = 'Dear '.ucfirst($data->DoctorName).', You have an appointment with '.ucfirst($data->PatientName).' at '.ucfirst($clinic_name1).' on '.$time.' at ' . $location .'%0aThank You%0aPhysio Plus Tech';				
					...
					$message_doctor = 'Dear '.ucfirst($data->DoctorName).', You have an appointment with '.ucfirst($data->PatientName).' at '.ucfirst($data->clinic_name).' on '.$time.' at ' . $location.'%0aThank You%0aPhysio Plus Tech';
		...
		if($query->num_rows() > 0){
			...
			if($data->StaffEmail != '' && $notify_to == 'doctor') {
				...
				$doctor = array(
					...
					'ClinicName' => ucfirst($data->clinic_name),
					'DoctorName' => ucfirst($data->DoctorName),
					'PatientName' => ucfirst($data->PatientfName).' '.ucfirst($data->PatientlName),
					'Location' => $location,
					'ClinicMobile' => $data->mobile
				);
	...
	public function smsNotify($app_id, $notify_to) {
		...
		if($query->num_rows() > 0){
			...
			$sms_count = $profile_info['sms_count'];
			$country = $profile_info['country'];
            $confirm=$profile_info['confirm_sms_notify'];
			if($data->PatientMobile != '' && $notify_to == 'patient') {
				if($this->session->userdata('client_id') == '1948') {
					$clinic_name1 = 'SUNSHINE SUPER SPECIALITY PHYSIOTHERAPY CLINIC';
					...
					$message_patient = 'Dear '.ucfirst($data->PatientName).', We are pleased to confirm your appointment at '.ucfirst($clinic_name1).' on '.$time.'. For Details contact ' . $mobile .'%0aThank You%0aPhysio Plus Tech';
					$patientSmsURL = 'https://www.smsgateway.center/SMSApi/rest/send?userId=klakshman&password=Smsgateway@2018&senderId=FISIYO&sendMethod=simpleMsg&msgType=TEXT&mobile='.$data->PatientMobile.'&duplicateCheck=true&format=json&msg='.urlencode($message_patient);
			    } else {
                if($confirm==1)
                {
					$message_patient = 'Dear '.ucfirst($data->PatientName).', We are pleased to confirm your appointment at '.ucfirst($data->clinic_name).' on '.$time.'. For Details contact ' . $mobile .'%0aThank You%0aPhysio Plus Tech';
					$patientSmsURL = 'https://www.smsgateway.center/SMSApi/rest/send?userId=klakshman&password=Smsgateway@2018&senderId=FISIYO&sendMethod=simpleMsg&msgType=TEXT&mobile='.$data->PatientMobile.'&duplicateCheck=true&format=json&msg='.urlencode($message_patient);
				}
                }
				$sms['patient'] = 'DONE';
	...
	public function smsAppCancelNotify($app_id, $notify_to) {
		...
		if($query->num_rows() > 0){
			...
			$sms_count = $profile_info['sms_count'];
			$country = $profile_info['country'];
            $cancel=$profile_info['cancel_sms_notify'];
			if($data->PatientMobile != '' && $notify_to == 'patient') {
				if($this->session->userdata('client_id') == '1948') {
					$clinic_name1 = 'SUNSHINE SUPER SPECIALITY PHYSIOTHERAPY CLINIC';
					//$message_patient = 'Dear '.ucfirst($data->PatientName).', We are pleased to confirm your appointment at '.ucfirst($clinic_name1).' on '.$time.'. For Details ' . $mobile;
					...
					//$message_patient = 'Dear '.ucfirst($data->PatientName).', Your appointment at '.ucfirst($clinic_name1).' on '.$time.'. has been cancelled. For any queries kindly call ' . $mobile .'.%0a-Physio Plus Tech';
					$message_patient = 'Dear '.ucfirst($data->PatientName).', Your appointment at '.ucfirst($clinic_name1).'  has been cancelled. For any queries kindly call ' . $mobile .'.%0a-Physio Plus Tech';
					$patientSmsURL = 'https://www.smsgateway.center/SMSApi/rest/send?userId=klakshman&password=Smsgateway@2018&senderId=FISIYO&sendMethod=simpleMsg&msgType=TEXT&mobile='.$data->PatientMobile.'&duplicateCheck=true&format=json&msg='.urlencode($message_patient);
			    } else {
					//$message_patient = 'Dear '.ucfirst($data->PatientName).', Your appointment at '.ucfirst($data->clinic_name).' on '.$time.'. has been cancelled. For any queries kindly call ' . $mobile .'.%0a-Physio Plus Tech';
					 
					$message_patient = 'Dear '.ucfirst($data->PatientName).', Your appointment at '.ucfirst($data->clinic_name).'  has been cancelled. For any queries kindly call ' . $mobile .'.%0a-Physio Plus Tech';
	...
	public function smsNotify_doc($app_id,$notify_to,$con_id) {
		...
		if($query->num_rows() > 0){
			...
			if($StaffMobile != '') {
				if($this->session->userdata('client_id') == '1948') {
					$clinic_name1 = 'SUNSHINE SUPER SPECIALITY PHYSIOTHERAPY CLINIC';
				$message_doctor = 'Dear '.ucfirst($DoctorName).', You have an appointment with '.ucfirst($data->PatientName).' at '.ucfirst($clinic_name1).' on '.$time.' at ' . $location .'%0aThank You%0aPhysio Plus Tech';				
				...
				$message_doctor = 'Dear '.ucfirst($DoctorName).', You have an appointment with '.ucfirst($data->PatientName).' at '.ucfirst($data->clinic_name).' on '.$time.' at ' . $location .'%0aThank You%0aPhysio Plus Tech';				
				$doctorSmsURL = 'https://www.smsgateway.center/SMSApi/rest/send?userId=klakshman&password=Smsgateway@2018&senderId=FISIYO&sendMethod=simpleMsg&msgType=TEXT&mobile='.$mobile_no.'&duplicateCheck=true&format=json&msg='.urlencode($message_doctor);
		...
		public function form_update($app_id)
		{
		    ...
		    $query = $this->db->get();
				if($query->num_rows() > 0){
				   ...
				   if($this->session->userdata('client_id') == '1948'){
					$clinic_name1 = 'SUNSHINE SUPER SPECIALITY PHYSIOTHERAPY CLINIC';
				    $message_patient = 'Dear '.ucfirst($data->PatientName).', Your appointment request at '.ucfirst($data->clinic_name).' is rescheduled to '.$time.' on '.$time1.', if you want to change it, kindly call '.$mobile.'%0aThank You%0aPhysio Plus Tech';
				    $patientSmsURL = 'https://www.smsgateway.center/SMSApi/rest/send?userId=klakshman&password=Smsgateway@2018&senderId=FISIYO&sendMethod=simpleMsg&msgType=TEXT&mobile='.$data->PatientMobile.'&duplicateCheck=true&format=json&msg='.urlencode($message_patient);
				   } else {
	...
	public function appointment_thatday() {
		...
		$this->db->select('ai.chat_room,ai.encounter_type,ai.order_id,ai.visit,ai.bill_id,ai.appointment_id,pi.first_name as pname,pi.last_name as pl_name,ai.start,ai.end,si.first_name as sname,si.last_name as sl_name,ai.staff_id,ai.item_id,pi.patient_code,pi.gender,pi.patient_id,ai.googleMeet_link as link,ai.item_id,pi.mobile_no,pi.google_review')->from('tbl_appointments ai');
		$this->db->join('tbl_patient_info pi','pi.patient_id=ai.patient_id');
		$this->db->join('tbl_staff_info si','si.staff_id=ai.staff_id');
		$query = $this->db->get();
		return ($query->num_rows() > 0) ? $query->result_array() : false;
