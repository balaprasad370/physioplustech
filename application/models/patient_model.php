...
class Patient_model extends CI_Model 
{
	...
	public function patient_info($sms_count,$sms_limit,$welcome_sms_notify)
	{
		...
		/* if($this->session->userdata('client_id') == '2047'){ */
				   $title = $this->input->post('first_name').' '.$this->input->post('last_name');
					...
					$this->db->select('clinic_name,logo,facebook,twitter,instagarm')->from('tbl_client')->where('client_id',$this->session->userdata('client_id'));
					$res = $this->db->get();
					...
					<table border="0" align="center" cellpadding="0" cellspacing="0" style=" font-family:Trebuchet MS, Arial, Helvetica, sans-serif; font-size:13px; background:#0099DC;">
						<tr>
							<td class="actions text-center">
												...
												<tr>
													<td class="actions text-center">
														<table width="570px" align="center" bgcolor="#FFFFFF">
															...
															<tr>
															<td  colspan = "2">
																<p style="color:#666;padding-left:25px;font-size:120%">We are pleased to welcome you to the '.$clinic_name.'.   </p>
															</td>
															</tr><tr>
															<td  colspan = "2">
															...
															<tr>
																<td  colspan = "2">
																	<p style="color:#666;padding-left:25px;font-size:120%">You can stay connected with us and get health tips through,</br>
																			...
																			<a href="'.$instagarm.'" >'.$instagarm.'</a><br>
																			</p>
																</td>
															</tr>
								...
								<p style="text-align:center; color:#CCC;">World`s Best Web-based Clinic Management Software for '.$clinic_name.'</p>
							</td>
						</tr>
					</table>
					</td>
					...
					$this->email->to($email);
					$this->email->subject('Congratulations! Welcome to '.$clinic_name);
	...
	public function pain_info($patient_id) {
		...
		$var = (count($_POST['diurnal_variations'])) >0 ? implode(',', $_POST['diurnal_variations']) : ""; 
		$pain_info = array(
			'client_id' => $this->session->userdata('client_id'),
			'patient_id' => $patient_id,
	...
	{
		if($from_date == '' && $to_date == ''){
			$where = array('tt.patient_id' => $patient_id, 'tt.client_id' => $this->session->userdata('client_id'),'treament_types'=>0);
		}else{
			$where = array('patient_id' => $patient_id, 'tt.client_id' => $this->session->userdata('client_id'), 'tt.treatment_date >=' => $from_date, 'tt.treatment_date <=' => $to_date,'treament_types'=>0);
		}
		$this->db->distinct();
	...
	public function addTreatmentTechniques($patient_id) {
		$staff_id = implode(',',$this->input->post("staff_id"));
		
		if($this->input->post('treatment_date')!='')
		{
		  ...
		  {
			  $numOfDays = $this->input->post("treatment_days");
			  if($numOfDays>0)
			  {
				  for($j=0;$j<$numOfDays;$j++)
				  {
					 if(count($_POST['treatments'])==1){
					  $treat = implode(',',$_POST['treatments']);
					$data=explode('|',$treat);	
					$treatments = $data[0];
					 }else{	 
					 $treat = implode(',',$_POST['treatments']);
							...
							$treatment_price = explode(',',$this->input->post('treatment_price'));
							$treatment_price1 = 0;
							foreach($treatment_price as $price)
							{
								$treatment_price1 +=  $price;
							}
							...
							if($res->result_array() != false){
								$treatmentInfo = array(
									...
									'treatments' => $treatments,
									'treatment_quantity' => $treatment_quantity,
									'treatment_price' => $treatment_price1,
									'staff_id' => $staff_id,
									'bill_status' => 0,
									'treatment_group_id' => $res->row()->treatment_group_id,
									'created_by' => $this->session->userdata('username'),
	...
	public function addTreatmentTechniques_old_06_21($patient_id) {
			  ...
			  if($numOfDays>0)
			  {
				  for($i=0;$i<$numOfDays;$i++)
				  {
					...
					for($j=0; $j  < count($t1); $j++)
					{
							...
							$this->db->insert('tbl_pati,ent_treatment_techniques', $treatmentInfo);
						} 
	...
	public function addSession($patient_id)
	{
		...
		for($i=0; $i < sizeof($id); $i++){
			$session_det = array(
				...
				'treatment_id' => $this->input->post('treatment_idss'),
        ...
        if($this->input->post('treatment_id')!=''){
		$this->db->where('client_id',$this->session->userdata('client_id'));
		$this->db->where('treatment_id',$this->input->post('treatment_id'));
		$this->db->set('treatment_date',date('Y-m-d',strtotime(str_replace('/','-',$this->input->post('sn_date')))));
   ...
   public function getPatientEPCount($patient_id)
	...
	public function viewTreatmentsDyas($patient_id,$from_date='',$to_date='')
	{
		if($from_date == '' && $to_date == ''){
			$where = array('tt.patient_id' => $patient_id, 'tt.client_id' => $this->session->userdata('client_id'),'treament_types'=>1);
		}else{
			$where = array('patient_id' => $patient_id, 'tt.client_id' => $this->session->userdata('client_id'), 'tt.treatment_date >=' => $from_date, 'tt.treatment_date <=' => $to_date,'treament_types'=>1);
		}
		
		/* $this->db->distinct();
		$this->db->select('GROUP_CONCAT(DISTINCT(tt.staff_id) SEPARATOR " - ") AS staff_id,S.sn_id,tt.client_id,tt.treatment_id,tt.patient_id,tt.treatment_date,tt.treatments,tt.treatment_quantity,tt.treatment_price,tt.treatment_group_id,tt.bill_status,GROUP_CONCAT(DISTINCT(it.item_name) SEPARATOR " - ") AS itemName,GROUP_CONCAT(DISTINCT(tt.treatment_quantity) SEPARATOR " - ") AS treatmentQuantity,GROUP_CONCAT(DISTINCT(tt.treatment_price) SEPARATOR " - ") AS treatmentPrice')->from('tbl_patient_treatment_techniques tt')->where($where)->group_by("tt.treatments");
		$this->db->join("tbl_item it", "tt.treatments=it.item_id");
		$this->db->join("tbl_session_det S", "tt.treatment_id=S.treatment_id","left");
		$this->db->order_by("tt.treatments", "asc");
		$query=$this->db->get(); */
		$this->db->distinct();
		$this->db->select('tt.staff_id AS staff_id,S.sn_id,tt.client_id,tt.treatment_id,tt.patient_id,tt.treatment_date,tt.treatments,tt.treatment_quantity,tt.treatment_price,tt.treatment_group_id,tt.bill_status,it.item_name AS itemName,tt.treatment_quantity AS treatmentQuantity,tt.treatment_price AS treatmentPrice')->from('tbl_patient_treatment_techniques tt')->where($where);
		$this->db->join("tbl_item it", "tt.treatments=it.item_id","left");
		$this->db->join("tbl_session_det S", "tt.treatment_id=S.treatment_id","left");
		//$this->db->join("tbl_staff_info SS", "tt.staff_id=SS.staff_id","inner");
		$this->db->order_by("tt.treatments", "asc");
		$query=$this->db->get();
		return ($query->num_rows() > 0) ? $query->result_array() : false;
	}
