	...
	public function add_visit($appointment_id){
		...
		$this->db->update('tbl_appointments');
		   ...
		   if($this->session->userdata('client_id') == '1948'){
				    ...
				    $message_patient = 'Dear '.ucfirst($data->PatientName).', Your appointment request at '.ucfirst($data->clinic_name).' is Confirmed to '.$time.' on '.$time1.', if you want to change it, kindly call '.$mobile.'%0aThank You%0aPhysio Plus Tech';
		...
		for($i=0; $i  < count($arrTreatmentDates); $i++)
		{
			if($arrTreatmentDates[$i]!='')
			{
				...
				for($j=0; $j  < count($t1); $j++)
				{
						...
						$treat_date=date('Y-m-d',strtotime(str_replace('/','-',$arrTreatmentDates[$i])));
						if($no_of_days=="0")
						{
							$treat_date=date('Y-m-d',strtotime(str_replace('/','-',$arrTreatmentDates[$i])));
						}
