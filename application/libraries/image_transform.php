...
//	var $cache = 'cache/'; // Cache folder
  ...
  function copyImage($image, $thumb = '') {
	...
	*/
	...
	/**
	* function crop_xy (integer $posX, integer $posY, string $image, integer $width, integer $height, string $thumb = '', boolean $view = false)
	*
	* Cut the image with the desired size. First the image is reduced or enlarged and after it's crop.
	*
	* If the parameter $thumb it's set, instead transform the image image,
	...
	* return boolean
	*/
	function crop_xy ($posX, $posY, $image, $width, $height, $thumb = '', $view = false) {
		if (!($this->data = $this->itsImage($image))) {
			return $this->debug($image.' isn\'t a valid image');
		}
	...
	*/
	...
	function progressive_jpeg($source){
		
		$ext = array_pop(explode('.',$source));
		$ext = strtolower($ext);
		$name_array = explode('.',$source);
		$name = '.'.$name_array[1];
		if($ext=='jpg' || $ext=='jpeg'){
